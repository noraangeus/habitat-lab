...
output_path = os.path.join(
    dir_path, "noras-habitat-lab/videos/"
)
...
with habitat.Env(
    ...
    # To save the video
    video_file_path = os.path.join(output_path, f"{now}.mp4")
...
video_file_path = os.path.join(output_path, f"{now}_gym.mp4")
video_writer = imageio.get_writer(video_file_path, fps=30)
...
with habitat.Env(
    ...
    count_steps = 0
    # To save the video
    video_file_path = os.path.join(output_path, f"{now}_yaml.mp4")
    video_writer = imageio.get_writer(video_file_path, fps=30)
